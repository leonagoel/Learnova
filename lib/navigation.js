...
export function getDashboardSidebarItems(role) {
  const dashboardHref = getDashboardLink(role);
  const primaryItems = [
    {
      href: dashboardHref,
      label: "Dashboard",
    ...
    { href: "/attendance", label: "Attendance", icon: UserCheck, key: "attendance" },
    { href: "/notices", label: "Notices", icon: Bell, key: "notices" },
  ];

  if (role === 'teacher') {
    primaryItems.push({ href: "/teacher/meetings", label: "Meetings", icon: Calendar, key: "meetings" });
  } else if (role === 'parent') {
    primaryItems.push({ href: "/parent/meetings", label: "Meetings", icon: Calendar, key: "meetings" });
  }
  ...
  return {
    primary: [
      ...
      ...(['teacher', 'parent'].includes(role) ? [
        { href: `/${role}/messages`, label: "Messages", icon: MessageCircle, key: "messages" }
      ] : []),
      ...(['teacher'].includes(role) ? [
        { href: `/teacher/behavior`, label: "Behavior Insights", icon: Activity, key: "behavior" }
      ] : []),
      ...(['student'].includes(role) ? [
        { href: `/student/career`, label: "Career Paths", icon: Compass, key: "career" },
        { href: `/student/pomodoro`, label: "Pomodoro Rooms", icon: Timer, key: "pomodoro" },
        { href: `/student/mentorship`, label: "Mentorship", icon: GraduationCap, key: "mentorship" },
        { href: `/student/models`, label: "STEM 3D Lab", icon: Box, key: "models" },
        { href: `/student/flashcards`, label: "AI Flashcards", icon: Brain, key: "flashcards" }
      ] : []),
      { href: `/${role}/assignments`, label: "Assignments", icon: BookOpen, key: "assignments" },
      { href: "/attendance", label: "Attendance", icon: UserCheck, key: "attendance" },
      { href: "/notices", label: "Notices", icon: Bell, key: "notices" },
      { href: `/${role}/meetings`, label: "Meetings", icon: Calendar, key: "meetings" },
      ...(['teacher', 'admin'].includes(role) ? [
        { href: `/${role}/polls`, label: "Live Polls", icon: BarChart, key: "polls" }
      ] : []),
    ],
    explore: NAVIGATION_ITEMS,
    account: [
