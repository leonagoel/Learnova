...
export async function GET(request) {
  try {
    ...
    const studentPolls = await Promise.all(polls.map(async (poll) => {
      const answerDoc = await db.collection('polls').doc(poll.id).collection('answers').doc(token.uid).get();
      ...
      return {
        ...
        submittedAnswer: answerDoc.exists ? answerDoc.data().answerIndex : null
      };
    }));
...
export async function POST(request) {
  try {
    ...
    const { question, options, correctAnswerIndex, xpReward } = await request.json();
    ...
    return NextResponse.json({ success: true, pollId: pollRef.id, message: 'Poll created successfully' });
  } catch (error) {
